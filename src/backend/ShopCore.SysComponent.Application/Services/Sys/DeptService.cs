...
public sealed class DeptService : RepositoryService<Sys_Dept, IDeptService>, IDeptService
{
    ...
    ///     创建部门
    /// </summary>
    /// <exception cref="ShopCoreInvalidOperationException">Parent_department_does_not_exist</exception>
    ...
    ///     删除部门
    /// </summary>
    /// <exception cref="ShopCoreInvalidOperationException">该部门下存在用户</exception>
    /// <exception cref="ShopCoreInvalidOperationException">该部门下存在子部门</exception>
    public async Task<int> DeleteAsync(DelReq req)
    {
        if (await Rpo.Orm.Select<Sys_User>().AnyAsync(a => a.DeptId == req.Id)) {
            throw new ShopCoreInvalidOperationException(Ln.该部门下存在用户);
        }

        #pragma warning disable IDE0046
        if (await Rpo.Select.AnyAsync(a => a.ParentId == req.Id)) {
            #pragma warning restore IDE0046
            throw new ShopCoreInvalidOperationException(Ln.该部门下存在子部门);
        }
    ...
    ///     更新部门
    /// </summary>
    /// <exception cref="ShopCoreUnexpectedException">ShopCoreUnexpectedException</exception>
    public async Task<QueryDeptRsp> UpdateAsync(UpdateDeptReq req)
    {
        if (await Rpo.UpdateDiy.SetSource(req).ExecuteAffrowsAsync() <= 0) {
            throw new ShopCoreUnexpectedException();
        }
