...
public sealed class RoleService : RepositoryService<Sys_Role, IRoleService>, IRoleService
{
    ...
    ///     删除角色
    /// </summary>
    /// <exception cref="ShopCoreInvalidOperationException">Users_exist_under_this_role_and_deletion_is_not_allowed</exception>
    public async Task<int> DeleteAsync(DelReq req)
    {
        return await Rpo.Orm.Select<Sys_UserRole>().ForUpdate().AnyAsync(a => a.RoleId == req.Id)
            ? throw new ShopCoreInvalidOperationException(Ln.该角色下存在用户)
